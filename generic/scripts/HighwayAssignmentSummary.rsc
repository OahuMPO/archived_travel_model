...
/*
	// Add the district field to the line layer using district field from the taz layer
    	...
    	Opts.Global.Fields = {llayers[2]+".DROPME"}                  // the field to fill
...
*/
	...
	DayAssign = scenarioDirectory+"\\outputs\\Allday.bin"
...
//  Create Vectors/Arrays to read data from tables/Dataviews
    ...
    totflow[1] = GetDataVector(VIEW2 + "|", "Tot_Flow",)                        // TOtal daily assigned flow for each link
    totVMT[1] = GetDataVector(VIEW2 + "|", "Tot_VMT",)                              // Total Daily Vehicles Miles Travelled
...
//  Initialize the Vectors that are used to estimate the statistics for comparing observed and assigned flows
    ...
    Volume_Groups = {
        ...
        {"10,000-15,000",10001,15000},
        {"15,000-20,000",15001,20000},
        {"20,000-25,000",20001,25000},
        {"25,000-50,000",25001,50000},
        {"50,000+",50001,999999}
        }
...
//	Counts Cross-check
    fp = OpenFile(scenarioDirectory+"\\Counts Cross-check.txt", "w")
...
CopyFile(scenarioDirectory+"\\outputs\\AM4HourFlow1.dcb",scenarioDirectory+"\\outputs\\Allday.dcb")
CopyFile(scenarioDirectory+"\\outputs\\AM4HourFlow1.bin", scenarioDirectory+"\\outputs\\Allday.bin")
...
dim abflow[3], baflow[3], totflow[3], abVMT[3], baVMT[3], totVMT[3]
    ...
    abVMT[1] = GetDataVector(AMPeak4 + "|", "AB_VMT",)
    baflow[1] = GetDataVector(AMPeak4 + "|", "BA_Flow",)
    baVMT[1] = GetDataVector(AMPeak4 + "|", "BA_VMT",)
    totflow[1] = GetDataVector(AMPeak4 + "|", "Tot_Flow",)
    totVMT[1] = GetDataVector(AMPeak4 + "|", "Tot_VMT",)

    // Next open the PmPeak table
    PMPeak4 = OpenTable("PMPeak4", "FFB", {scenarioDirectory+"\\outputs\\PM4HourFlow1.bin"}, {{"Shared", "True"}})

    // get the vector for time and then flow in the ab direction
    abflow[2] = GetDataVector(PMPeak4 + "|", "AB_Flow",)
    abVMT[2] = GetDataVector(PMPeak4 + "|", "AB_VMT",)
    baflow[2] = GetDataVector(PMPeak4 + "|", "BA_Flow",)
    baVMT[2] = GetDataVector(PMPeak4 + "|", "BA_VMT",)
    totflow[2] = GetDataVector(PMPeak4 + "|", "Tot_Flow",)
    totVMT[2] = GetDataVector(PMPeak4 + "|", "Tot_VMT",)

    // Next open the OffPeak table
    OffPeak16 = OpenTable("OffPeak16", "FFB", {scenarioDirectory+"\\outputs\\OffpeakFlow3.bin"}, {{"Shared", "True"}})

    // get the vector for time and then flow in the ab direction
    abflow[3] = GetDataVector(OffPeak16 + "|", "AB_Flow",)
    abVMT[3] = GetDataVector(OffPeak16 + "|", "AB_VMT",)
    baflow[3] = GetDataVector(OffPeak16 + "|", "BA_Flow",)
    baVMT[3] = GetDataVector(OffPeak16 + "|", "BA_VMT",)
    totflow[3] = GetDataVector(OffPeak16 + "|", "Tot_Flow",)
    totVMT[3] = GetDataVector(OffPeak16 + "|", "Tot_VMT",)

    // First you need to open the table
    AllDay = OpenTable("AllDay", "FFB", {scenarioDirectory+"\\outputs\\Allday.bin"}, {{"Shared", "True"}})

    // set the vectors
    SetDataVector(AllDay + "|", "AB_Flow", abflow[1] + abflow[2] + abflow[3],)
    SetDataVector(AllDay + "|", "AB_VMT", abVMT[1] + abVMT[2] + abVMT[3],)
    SetDataVector(AllDay + "|", "BA_Flow", baflow[1] + baflow[2] + baflow[3],)
    SetDataVector(AllDay + "|", "BA_VMT", baVMT[1] + baVMT[2] + baVMT[3],)
    SetDataVector(AllDay + "|", "Tot_Flow", totflow[1] + totflow[2] + totflow[3],)
    SetDataVector(AllDay + "|", "Tot_VMT", totVMT[1] + totVMT[2] + totVMT[3],)
