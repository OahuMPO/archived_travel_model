Macro "AM Highway Assignment Summary"
...
//	if !ret_value then Throw()
	...
	DayAssign = ScenarioDirectory + "outputs\\AM4HourFlow1.bin"
...
//  Create Vectors/Arrays to read data from tables/Dataviews
    ...
    AADT[1] = GetDataVector(VIEW2 + "|", "[AM Peak]",)                                   // Count Data
...
//          Which are:  Obs_Sta_Count & Est_Sta_Count
    for i = 1 to MaxCountID do
        if Obs_Sta_Count[2][i] <> null then do
            NumLinkCS[Obs_Sta_Count[1][i]][Obs_Sta_Count[3][i]] = NumLinkCS[Obs_Sta_Count[1][i]][Obs_Sta_Count[3][i]] + Links_Per_Station[i]
...
//	Counts Cross-check
    fp = OpenFile(ScenarioDirectory + "AM_Peak_Counts Cross-check.txt", "w")
...
// Print out the Tables
  ...
  fp = OpenFile(ScenarioDirectory + "AM_Peak_Highway Assignment Summary.txt", "w")
	WriteLine(fp, "Total Daily Highway Assignment Results for OMPO Model")
	Writeline(fp, "")						//--- a blank line
	WriteLine(fp, "Links with Counts")
